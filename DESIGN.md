...
**OWN-WORLD.** Dark, tactile, cinematic landscapes and material studies: midnight navy, cool graphite, forest green, smoked violet and restrained ivory light. Every composition concentrates detail on the right and preserves a calm dark field beneath copy. The Player opens the chosen sound as a full cinematic place; one ink-blue transport control bridges artwork into the operational surface.
...
- **Radius** (`Radius`) — `tag: 4`, `card: 10`, `sheet: 20`. Flatter than the old system's pill/circle defaults; genuinely circular elements such as icon badges and transport controls opt in explicitly.
...
- `assets/images/art/presets/*.jpg` — 33 original ChatGPT-generated scenes/material studies, one per preset.
- `assets/images/art/states/*.jpg` — authored operational empty-state artwork, currently Favorites.
- `assets/images/art/light/**/*.jpg` — composition-matched cool-daylight edits for all 38 semantic scenes. Production and edit prompts live in `docs/ART-ASSET-BRIEFS.md`.
